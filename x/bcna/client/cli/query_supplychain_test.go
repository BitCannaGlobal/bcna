...
func networkWithSupplychainObjects(t *testing.T, n int) (*network.Network, []types.Supplychain) {
	t.Helper()
	...
	return network.New(t, cfg), state.SupplychainList
}

func TestShowSupplychain(t *testing.T) {
	net, objs := networkWithSupplychainObjects(t, 2)
	...
	for _, tc := range tests {
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowSupplychain(), args)
			if tc.err != nil {
...
func TestListSupplychain(t *testing.T) {
	net, objs := networkWithSupplychainObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListSupplychain(), args)
			require.NoError(t, err)
