...
func networkWithBitcannaidObjects(t *testing.T, n int) (*network.Network, []types.Bitcannaid) {
	t.Helper()
	...
	return network.New(t, cfg), state.BitcannaidList
}

func TestShowBitcannaid(t *testing.T) {
	net, objs := networkWithBitcannaidObjects(t, 2)
	...
	for _, tc := range tests {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowBitcannaid(), args)
			if tc.err != nil {
...
func TestListBitcannaid(t *testing.T) {
	net, objs := networkWithBitcannaidObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListBitcannaid(), args)
			require.NoError(t, err)
